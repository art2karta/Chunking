...
- Делит текст по иерархии заголовков:
  - сначала по `##` (H2)
  - если H2-секция слишком длинная, делит по `###` (H3)
  - если и после этого секция слишком длинная (и без кода), применяет `RecursiveCharacterTextSplitter`
- Собирает финальные чанки с ограничениями по токенам
- Добавляет адаптивный overlap между соседними чанками
- Сохраняет результат в двух форматах в папку `chunks_output`:
  - `.txt` (читаемый отчет)
  - `.jsonl` (по одному JSON-объекту на чанк)
...
- Стандартная библиотека Python: `re`, `json`, `pathlib`
...
- `MarkdownHeaderTextSplitter` извлекает структуру `H1/H2/H3`
- Базовая логика чанкинга:
  - сначала попытка взять целиком секцию `H2`
  - если секция `H2` не помещается, деление по `H3`
  - если конкретная `H3`-секция все еще не помещается (и не содержит кода), применяется `RecursiveCharacterTextSplitter`
...
- Если чанк содержит код-блок, он не режется принудительно по токен-лимиту
- Чанк с кодом может быть очень большим (например, 10 000+ токенов)
- Для таких чанков сохраняется стандартный overlap с предыдущим чанком
...
В текущем алгоритме приоритет у семантических границ `H2/H3`, поэтому принудительное объединение маленьких чанков не используется.
...
- `.txt` содержит:
  ...
  - сами чанки
- `.jsonl` содержит по одной записи на строку с полями:
  - `chunk_index`
  - `document`
  - `h1`
  - `h2`
